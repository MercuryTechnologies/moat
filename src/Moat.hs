...
enumExp parentName tyVars protos cases raw tags bs
  = applyBase bs $ RecConE 'MoatEnum
      ...
      , (mkName "enumInterfaces", protosExp protos)
...
structExp name tyVars protos fields tags bs
  = applyBase bs $ RecConE 'MoatStruct
      ...
      , (mkName "structInterfaces", protosExp protos)
...
giveProtos ps = \case
  s@MoatStruct{} -> s { structInterfaces = ps }
  s@MoatEnum{} -> s { enumInterfaces = ps }
  s -> s
...
giveBase :: Maybe MoatType -> [Protocol] -> SwiftData -> SwiftData
giveBase r ps = \case
